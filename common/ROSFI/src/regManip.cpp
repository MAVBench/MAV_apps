...
#define MAX_INSTR_BYTES 16
  ...
  if (isIn(InjectTo, "i")) {
    printf("get implicitly accessed register\n");
    ...
    printf("finish get implicitly accessed register\n");
  }

  // 3. Explicitly accessed registers.
  if (isIn(InjectTo, "e")) {
    printf("collect Explicitly accessed registers.\n");
    ...
    for (int Idx = 0, E = X86Data.op_count; Idx != E; ++Idx) {
      ...
      case X86_OP_REG: {
        ...
        if (Operand.access & CS_AC_WRITE) {
          printf("collect write register\n");
          RDescr.Written = true;
          WRegs.push_back(RDescr);
        } else if (Operand.access & CS_AC_READ) {
          printf("collect read register\n");
  ...
  ROS_INFO("size of AllRegs: %d ", AllRegs.size());
  for (int i = 0; i < AllRegs.size(); ++i)
  {
    ROS_INFO("name: %s", AllRegs[i].Name.c_str());
  }
  cs_free(Instrs, Cnt);
  cs_close(&handle);
...
RegisterManipulator::getRandomRegAndBit(uint8_t *IP) {
  ...
  return std::make_tuple(Reg, StartBit, true);
}
...
bool RegisterManipulator::tryBitFlip(const std::string &Reg, unsigned Bit) {
  ...
  if(!(StrToRegMap.count(Reg))){
    ROS_INFO("Missing register from map");
  }
  importRegisters();
