...
class Model(object):
	...
	# for that to work curve_fit must be called with a p0 so it can deduce the number of coefficients #
	# also need timing function
	def fit(self, xdata, ydata, p0, profile=False, *args, **kwargs):
		if (profile):
			t0 = time.time()
