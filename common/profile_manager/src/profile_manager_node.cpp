...
vector<KeyValueStringPairStruct> g_highlevel_application_stats_verbose;
...
void output_flight_summary(void){
   ...
   for (auto result_el: g_highlevel_application_stats_verbose) {
        stats_ss<<  "\t\""<< result_el.key<<'"' <<": " << result_el.value<<"," << endl;
    }
...
bool record_profiling_data_cb(profile_manager::profiling_data_srv::Request &req, profile_manager::profiling_data_srv::Response &res)
{
    ...
    ROS_ERROR_STREAM("call back function was called with "<<req.key);
...
int main(int argc, char **argv)
{
    ...
    ros::ServiceServer record_profiling_data_verbose_service = nh.advertiseService("record_profiling_data_verbose", record_profiling_data_verbose_cb);
