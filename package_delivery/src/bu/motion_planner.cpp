...
void create_response_unsmoothened(package_delivery::get_trajectory::Response &res, piecewise_trajectory un_smooth_path);
...
bool get_trajectory_fun(package_delivery::get_trajectory::Request &req, package_delivery::get_trajectory::Response &res)
{
    ...
    */
    ...
    ROS_INFO_STREAM("graph size"<<piecewise_path.size()); 
    create_response_unsmoothened(res, piecewise_path);
    // Publish the trajectory (for debugging purposes)
    traj_topic = res.multiDOFtrajectory;
    //ROS_INFO_STREAM(res.size());
...
void create_response_unsmoothened(package_delivery::get_trajectory::Response &res, piecewise_trajectory un_smooth_path)
{
   ...
   //mav_trajectory_generation::sampleWholeTrajectory(smooth_path, sampling_interval__global, &states);
    ...
    res.unknown = -1;
    graph::node prev_s = un_smooth_path[0];
    int state_index = 0;
	for (const auto& s : un_smooth_path) {
		trajectory_msgs::MultiDOFJointTrajectoryPoint point;

		geometry_msgs::Transform pos;
        graph::node current;
		pos.translation.x = current.x = s.x;
		pos.translation.y = current.y = s.y;
		pos.translation.z = current.z = s.z;

		geometry_msgs::Twist vel;
		vel.linear.x = (s.x - prev_s.x)/.02;
		vel.linear.y = (s.y - prev_s.y)/.02;
		vel.linear.z = (s.z - prev_s.z)/.02;

        ROS_INFO_STREAM("vels"<<vel.linear.x << " " <<vel.linear.y<<" " <<vel.linear.z);
