...
bool MotionPlanner::get_trajectory_fun(package_delivery::get_trajectory::Request &req, package_delivery::get_trajectory::Response &res)
{
    ...
    if (DEBUG__global){
        ...
        marker.id = cnt++;
        marker.pose.position.x = req.start.x;
        marker.pose.position.y = req.start.y;
        marker.pose.position.z = req.start.z;
        marker.scale.x = 1.0;
        marker.scale.y = 1.0;
        marker.scale.z = 1.0;
        marker.color.a = 1.0;
        marker.color.b = 1.0;
        start_point_pub.publish(marker);
    }
    ...
    if (DEBUG__global){ 
        ...
        for (int i =0; i<piecewise_traj_markers.markers.size(); i++){
            piecewise_traj_markers.markers[i].action = visualization_msgs::Marker::DELETE;
        }
    }
...
bool MotionPlanner::collision(octomap::OcTree * octree, const graph::node& n1, const graph::node& n2, graph::node * end_ptr)
{
            ...
            end_it = octree->end_leafs_bbx(); it != end_it; ++it)
    {
        ...
        if (direction_norm(0) == 0 &&
                direction_norm(1) == 0 &&
                direction_norm(2) == 0){
            ROS_INFO_STREAM("norm is too small");
            fflush(stdout); 
        }
...
MotionPlanner::smooth_trajectory MotionPlanner::smoothen_the_shortest_path(piecewise_trajectory& piecewise_path, octomap::OcTree* octree, Eigen::Vector3d initial_velocity, Eigen::Vector3d initial_acceleration)
{
	...
	//start_v.makeStartOrEnd(Eigen::Vector3d(piecewise_path.front().x, piecewise_path.front().y, piecewise_path.front().z), derivative_to_optimize);
   	ROS_INFO_STREAM("------------------------------- initial velocity"<<initial_velocity[0]<<" "<< initial_velocity[1]<<" " << initial_velocity[2]);
    ...
    do {
        ...
        //ROS_INFO_STREAM("got all the segments");
        ...
        /* 
        if(ros::Time::now() > g_start_time+ros::Duration(g_planning_budget)){
             ROS_INFO_STREAM("exceeded the timing budget for planning");
             fflush(stdout);
         }
         */
       smoothener_ctr++; 
    } while (col && smoothener_ctr > 400);
   // &&
    //        ros::Time::now() < g_start_time+ros::Duration(g_planning_budget));

    if (col){
        if (DEBUG__global){ 
            ROS_INFO_STREAM("col was true when terminated");
            fflush(stdout);
            ...
            for (int i =0; i<piecewise_traj_markers.markers.size(); i++){
                piecewise_traj_markers.markers[i].lifetime= ros::Duration(4);
            }
            smooth_failed_traj_vis_pub.publish(smooth_traj_markers);
        }
